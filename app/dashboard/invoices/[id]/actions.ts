...
export async function updateProjectStatusWithPayment(
  ...
  for (const inv of invoices) {
    ...
    if (paymentInsertError) {
      // On log l'erreur côté serveur (visible dans les logs Vercel) plutôt que
      // de la laisser échouer silencieusement — utile pour diagnostiquer un
      // souci de policy RLS ou de colonne manquante sur la table payments.
      console.error("Erreur insertion payments:", paymentInsertError.message);
    }
    ...
    // Revalide explicitement la page de détail de cette facture précise,
    // pour que le nouveau paiement apparaisse immédiatement sans cache périmé.
